...
var Jiffy = function (){
  ...
  return{
	...
	measure : function(eventName, referenceID){
		...
		// If the referenceID is not passed in then it is from one of the window events
		// Set to 'PageStart'
		if(typeof(referenceID) == "undefined" || referenceID == null) {
			referenceID = "PageStart";
		}

		// If there is a previous measure then use it for reference
		if(markers[referenceID] != null) {
			refStartTime = markers[referenceID].lastTime;
			elapsedTime = currTime - refStartTime;
			markers[referenceID].lastTime = currTime;
		}
		// otherwise create a "pseudo mark" for this
		else {
